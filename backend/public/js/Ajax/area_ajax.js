$(document).ready(function(){
  ...
  /*********************************      USUARIOS       ************************************/
    ...
    /*****************************************************************************************/
    $("#retorno_index").hide();
    if(router == "area/registar"){
   
      $("a[name='bt_area_alterar'").click(function(){              
        ajax_enviar_dados2()
      })            
      function ajax_enviar_dados2(){

        var nome = $("input[id='titulo']").val().replace( /[\/]/g, "" );
        var status = $('input[name=area_status]:checked').val();
        ...
        if(false){
          ...
          $.ajax({
              ...
              type: "POST",
              data: {'url':'area/inserir/nome/'+nome+'/status/'+status+'/'},
              success: function(alterar){
                ...
                location.replace("/backend/area/listar");
              }
          });
        }
      } 
      ...
      function ajax_enviar_dados2(){
        ...
        if(false){
          ...
          $.ajax({
              url: "/backend/Ajax.php",
              type: "POST",
              data: {'url':'area/alteracao/id/'+id_inst+'/nome/'+nome+'/status/'+status+'/'},
              success: function(alterar){
                new PNotify({
                  title: translate('Sucesso!'),
                  text: alterar,
    ...
    /***********************                DELETAR                  *************************/
    /*****************************************************************************************/
    if(router == "area/listar"){

      
      $("table.dataTable").on('click',"a.areaDel",function(e){
