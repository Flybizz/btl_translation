...
class App_gapi{
  ...
  public function __construct($client_email, $key_file, $delegate_email = null) {
    ...
    $this->auth_method = new App_gapiOAuth2();
  ...
  public function requestAccountData($start_index=1, $max_results=1000) {
    ...
    $url = new App_gapiRequest(App_gapi::account_data_url);
  ...
  public function requestReportData($report_id, $dimensions=null, $metrics, $sort_metric=null, $filter=null, $start_date=null, $end_date=null, $start_index=1, $max_results=10000) {
    ...
    $parameters['prettyprint'] = App_gapi::dev_mode ? 'true' : 'false';
    
    $url = new App_gapiRequest(App_gapi::report_data_url);
  ...
  protected function accountObjectMapper($json_string) {
    ...
    foreach ($json['items'] as $item) {
      foreach ($item['webProperties'] as $property) {
        ...
        $results[] = new App_gapiAccountEntry($property);
      }
    }
  ...
  protected function reportObjectMapper($json_string) {
    ...
    if(isset($json['rows'])){
      foreach($json['rows'] as $row) {
        ...
        $results[] = new App_gapiReportEntry($metrics, $dimensions);
      }
    }
  ...
  public function __call($name, $parameters) {
    ...
    $parameter_key = App_gapi::ArrayKeyExists($name, $this->report_root_parameters);

    if ($parameter_key) {
      return $this->report_root_parameters[$parameter_key];
    }

    $aggregate_metric_key = App_gapi::ArrayKeyExists($name, $this->report_aggregate_metrics);
...
class App_gapiAccountEntry {
  ...
  public function __call($name, $parameters) {
    ...
    $property_key = App_gapi::ArrayKeyExists($name, $this->properties);
...
class App_gapiReportEntry {
  ...
  public function __call($name, $parameters) {
    ...
    $metric_key = App_gapi::ArrayKeyExists($name, $this->metrics);

    if ($metric_key) {
      return $this->metrics[$metric_key];
    }

    $dimension_key = App_gapi::ArrayKeyExists($name, $this->dimensions);
...
class App_gapiOAuth2 {
  ...
  public function fetchToken($client_email, $key_file, $delegate_email = null) {
    ...
    $url = new App_gapiRequest(self::request_url);
 ...
 *
 */
class App_gapiRequest {
  const http_interface = 'auto'; //'auto': autodetect, 'curl' or 'fopen'
  const interface_name = App_gapi::interface_name;
