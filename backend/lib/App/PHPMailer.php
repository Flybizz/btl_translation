...
class App_PHPMailer
{
    ...
    protected function addOrEnqueueAnAddress($kind, $address, $name)
    {
        ...
        if (false === $pos) {
            ...
            if ($this->exceptions) {
                throw new App_Exception($error_message);
            }
    ...
    public function preSend()
    {
        ...
        try {
            ...
            if (count($this->to) + count($this->cc) + count($this->bcc) < 1) {
                throw new App_Exception($this->lang('provide_address'), self::STOP_CRITICAL);
            }
            ...
            if (!$this->AllowEmpty and empty($this->Body)) {
                throw new App_Exception($this->lang('empty_message'), self::STOP_CRITICAL);
            }
    ...
    protected function sendmailSend($header, $body)
    {
        ...
        if ($this->SingleTo) {
            foreach ($this->SingleToArray as $toAddr) {
                ...
                if (!$mail) {
                    throw new App_Exception($this->lang('execute') . $this->Sendmail, self::STOP_CRITICAL);
                }
    ...
    protected function mailSend($header, $body)
    {
        ...
        if (!$result) {
            throw new App_Exception($this->lang('instantiate'), self::STOP_CRITICAL);
        }
    ...
    protected function smtpSend($header, $body)
    {
        ...
        if (!$this->smtpConnect($this->SMTPOptions)) {
            throw new App_Exception($this->lang('smtp_connect_failed'), self::STOP_CRITICAL);
        }
        ...
        if (!$this->smtp->mail($smtp_from)) {
            ...
            throw new App_Exception($this->ErrorInfo, self::STOP_CRITICAL);
        }
        ...
        if ((count($this->all_recipients) > count($bad_rcpt)) and !$this->smtp->data($header . $body)) {
            throw new App_Exception($this->lang('data_not_accepted'), self::STOP_CRITICAL);
        }
    ...
    public function smtpConnect($options = null)
    {
        ...
        foreach ($hosts as $hostentry) {
            ...
            if ('tls' === $secure or 'ssl' === $secure) {
                ...
                if (!$sslext) {
                    throw new App_Exception($this->lang('extension_missing') . 'openssl', self::STOP_CRITICAL);
                }
            }
            $host = $hostinfo[3];
            $port = $this->Port;
                ...
                try {
                    ...
                    if ($tls) {
                        if (!$this->smtp->startTLS()) {
                            throw new App_Exception($this->lang('connect_host'));
                        }
                    ...
                    if ($this->SMTPAuth) {
                        if (!$this->smtp->authenticate(
                            ...
                            throw new App_Exception($this->lang('authenticate'));
                        }
                    }
        ...
        if ($this->isError()) {
            ...
            try {
                if (!defined('PKCS7_TEXT')) {
                    throw new App_Exception($this->lang('extension_missing') . 'openssl');
                }
                // @TODO would be nice to use php://temp streams here
                $file = tempnam(sys_get_temp_dir(), 'mail');
                if (false === file_put_contents($file, $body)) {
                    throw new App_Exception($this->lang('signing') . ' Could not write temp file');
                }
                ...
                if ($sign) {
                    ...
                    @unlink($signed);
                    throw new App_Exception($this->lang('signing') . openssl_error_string());
                }
            } catch (Exception $exc) {
                $body = '';
                if ($this->exceptions) {
    ...
    public function addAttachment($path, $name = '', $encoding = self::ENCODING_BASE64, $type = '', $disposition = 'attachment')
    {
        try {
            if (!static::isPermittedPath($path) || !@is_file($path)) {
                throw new App_Exception($this->lang('file_access') . $path, self::STOP_CONTINUE);
            }
    ...
    protected function encodeFile($path, $encoding = self::ENCODING_BASE64)
    {
        try {
            if (!static::isPermittedPath($path) || !file_exists($path)) {
                throw new App_Exception($this->lang('file_open') . $path, self::STOP_CONTINUE);
            }
